...
/// </summary>
public class NKeysReference2 : IDisposable
{
    private byte[] _seed;
    ...
    internal NKeysReference2(byte[] userSeed)
    {
        if (userSeed == null)
    ...
    public void Wipe()
    {
        NKeysUtilsReference2.Wipe(ref _seed);
        NKeysUtilsReference2.Wipe(ref _expandedPrivateKey);
    }
...
/// </summary>
public class NKeysUtilsReference2
{
    ...
    public static byte[] Decode(string src)
    {
        ...
        if (crc != NKeysReference2Crc16.Checksum(data))
    ...
    public static NKeysReference2 FromSeed(string seed)
    {
        var userSeed = DecodeSeed(seed);
        try
        {
            var kp = new NKeysReference2(userSeed);
            return kp;
        }
        finally
        {
    ...
    {
        var s = NKeysUtilsReference2.Decode(seed);
    ...
    {
        return DecodeSeed(NKeysUtilsReference2.Decode(src));
    }
    ...
    internal static string Encode(byte prefixbyte, bool seed, byte[] src)
    {
        ...
        var checksum = BitConverter.GetBytes(NKeysReference2Crc16.Checksum(stream.ToArray()));
        stream.Write(checksum, 0, checksum.Length);

        return NKeysReference2Base32.Encode(stream.ToArray());
    }
...
internal static class NKeysReference2Crc16
{
...
internal class NKeysReference2Base32
{
